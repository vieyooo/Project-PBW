...
class PelangganController extends Controller
{
    public function index(Request $request)
    {
        ...
        $pelanggans = Pelanggan::orderBy('ID_PELANGGAN')->paginate($limit);
    ...
    public function create()
{
    ...
    if ($max_id) {
        $angka = (int) substr($max_id, 4); // ambil angka setelah 'PLG-'
        $angka++;
    } else {
        $angka = 2001;
    }
    ...
    public function update(Request $request, $id)
{
    ...
    $pelanggan = Pelanggan::findOrFail($id);
    $pelanggan->update($request->only(['nama_pelanggan','alamat','no_telp','fax']));
