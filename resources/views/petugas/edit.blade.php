...
@section('content')
<style>
    /* (sama seperti CSS di create, ditambah style untuk .ttd-current, .checkbox-group) */
    .form-container {
        max-width: 600px;
        margin: 0 auto;
        background: white;
...
<div class="form-container">
    <div class="form-header">
        <h3><i class="fas fa-user-edit"></i> Edit Petugas</h3>
    </div>
    @if ($errors->any())
        <div class="alert-error">{{ $errors->first() }}</div>
    @endif

    <form method="POST" action="{{ route('petugas.update', $petugas->ID_PETUGAS) }}" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="form-group">
            <label class="form-label">ID Petugas</label>
            <input type="text" class="form-control" value="{{ $petugas->ID_PETUGAS }}" readonly>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select name="JABATAN" class="form-control" required>
                ...
                @foreach($jabatanOptions as $jab)
                    <option value="{{ $jab }}" {{ $petugas->JABATAN == $jab ? 'selected' : '' }}>{{ $jab }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            @if($petugas->FILE_TTD && file_exists(public_path('img/ttd/' . $petugas->FILE_TTD)))
                <div class="ttd-current">
                    <div><img src="{{ asset('img/ttd/' . $petugas->FILE_TTD) }}" alt="Tanda Tangan"></div>
                    <div class="checkbox-group">
                        <input type="checkbox" name="hapus_ttd" id="hapus_ttd" value="1">
                        <label for="hapus_ttd"><i class="fas fa-trash-alt"></i> Hapus tanda tangan</label>
                    </div>
                </div>
            @endif
            ...
            <div class="file-input-wrapper" style="margin-top: 10px;">
                <label class="file-input-label" for="ttd-input">
                    <i class="fas fa-upload"></i> {{ $petugas->FILE_TTD ? 'Ganti File' : 'Pilih File' }}
                </label>
            ...
            <div class="ttd-preview" id="ttd-preview">
                <img id="preview-img" src="#" alt="Preview Tanda Tangan Baru">
                <p class="form-hint">Preview tanda tangan baru</p>
            </div>
            <div class="form-hint">
                <i class="fas fa-info-circle"></i> Format: JPG, JPEG, PNG. Maksimal 2MB. Kosongkan jika tidak ingin mengubah.
            </div>
        </div>
    ...
    if (ttdInput) {
        ttdInput.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                if (hapusTtdCheckbox) hapusTtdCheckbox.checked = false;
            } else {
